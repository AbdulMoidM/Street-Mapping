...
public class StreetMap {
	...
	public static void drawFrame() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(n!=null) {//until all paths of route are traversed
					if(n.get(n.size()-1) == null) {
						...
						startLoc.setText("Starting Location(Yellow): "+ location1);//will be printed in south panel of JFrame
						endLoc.setText("Ending Location(Purple): "+ location2);//will be printed in south panel of JFrame
					}else {
						DecimalFormat df = new DecimalFormat("#.###");
						formatted = df.format(n.get(n.size()-1).distToSource); 
						dist.setText("Distance(miles): "+formatted);
						startLoc.setText("Starting Location(Yellow): "+ location1);
						endLoc.setText("Ending Location(Purple): "+ location2);
					}
				}
			}
		});
		...
		startLoc = new JLabel("Starting Location(Yellow): "+ firstLocation);//label will go in south panel 
		endLoc = new JLabel("Ending Location(Purple): "+ secondLocation);//label will go in south panel
		System.out.println();
		DecimalFormat df = new DecimalFormat("#.###");
		dist=null;
		if(n!=null) {
